...
ENV DOCKER_RUN 'echo "$(sudo docker rm -f HAPROXY-DOCKER) $(sudo docker run -d -p 80:80 -v /usr/bin/docker:/usr/bin/docker --name=HAPROXY-INTERNAL --restart=always dockerimages/haproxy-docker)"'
