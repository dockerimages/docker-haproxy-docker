...
ENV DOCKER_RUN "sudo docker rm -f HAPROXY-INTERNAL && sudo docker run -d -p 80:80 -v /usr/bin/docker:/usr/bin/docker --name=HAPROXY-INTERNAL --restart=always dockerimages/haproxy-internal"
