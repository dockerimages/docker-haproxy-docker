...
ENV DOCKER_RUN /bin/bash -c "echo $(sudo docker rm -f HAPROXY-DOCKER) $(sudo docker run -d -p 80:80 -v /usr/bin/docker:/usr/bin/docker --name=HAPROXY-DOCKER --restart=always dockerimages/haproxy-docker)"
